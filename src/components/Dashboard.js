...
export function Dashboard() {
  const [sensorData, setSensorData] = useState({
    gps: { lat: 0, lng: 0, address: "Loading..." },
    heartbeat: 0,
    motion: 0,
    sound: 0
  });
  ...
  useEffect(() => {
    ...
    const sensorRef = ref(db, "sensor_data/latest"); // adjust path to your DB
  ...
  const checkThresholds = (data) => {
    ...
    if (data.heartbeat < thresholds.heartbeat.low) {
      newNotifications.push({
        ...
        sensor: "Heart Rate",
        message: `Low heart rate detected: ${data.heartbeat} BPM`,
        timestamp: new Date(),
        type: "warning"
      });
      ...
      newNotifications.push({
        ...
        sensor: "Heart Rate",
        message: `High heart rate detected: ${data.heartbeat} BPM`,
        timestamp: new Date(),
        type: "alert"
      });
    }
    ...
    if (data.motion > thresholds.motion.high) {
      newNotifications.push({
        ...
        message: `High motion detected: ${data.motion}`,
        timestamp: new Date(),
        type: "alert"
      });
    }
    ...
    if (data.sound > thresholds.sound.high) {
      newNotifications.push({
        ...
        sensor: "Sound Level",
        message: `High noise level: ${data.sound} dB`,
        timestamp: new Date(),
        type: "warning"
      });
    }
